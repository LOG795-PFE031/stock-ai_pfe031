        ...
        for symbol in symbols:
            try:
                ...
                if result.get("status") == "success":
                    logger['training'].info(f"Successfully trained Prophet model for {symbol}")
                    logger['training'].info(f"Model metrics: {result.get('result', {}).get('metrics', {})}")
                else:
                    error_msg = result.get("error", "Unknown error")
                    logger['training'].error(f"Failed to train Prophet model for {symbol}: {error_msg}")
            except Exception as e:
                logger['training'].error(f"Error training Prophet model for {symbol}: {str(e)}")
                continue
