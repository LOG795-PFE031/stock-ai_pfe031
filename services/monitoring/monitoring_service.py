...
class MonitoringService(BaseService):
    ...
    async def _run_performance_check(self) -> None:
        """
        Evaluate each live model's MAE once per day and retrain if performance degraded.
        """
        self.logger.info("🔍 Starting daily performance drift check")
        try:
            ...
            for name in models:
                ...
                self.logger.debug(f"Checking performance for {model_type}_{symbol}")
                ...
                if mae is None:
                    self.logger.warning(
                        f"No MAE for {name}, skipping performance drift check"
                    )
                    continue

                prev = self._last_mae.get((model_type, symbol))
                self._last_mae[(model_type, symbol)] = mae
                if prev is not None:
                    if mae > prev:
                        self.logger.info(
                            f"MAE ↑ for {name}: {prev:.4f} → {mae:.4f}, retraining"
                        )
                        ...
                        self.logger.info(
                            f"No performance retraining needed for {name}: {prev:.4f} → {mae:.4f}"
                        )
        except Exception as e:
            self.logger.error(
                f"Error in daily performance check: {e}", exc_info=True
            )
        finally:
            self.logger.info("✅ Daily performance drift check complete")
    ...
    async def _check_data_drift(self, model_type: str, symbol: str) -> None:
        ...
        try:
            ...
            # Compare against threshold & retrain if needed
