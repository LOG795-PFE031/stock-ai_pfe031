...
class NewsService(BaseService):
    ...
    def __init__(self):
        ...
        try:
            ...
            logger.warning(f"Failed to initialize FinBERT, falling back to TextBlob: {str(e)}")
        ...
        try:
            ...
            try:
                ...
                def textblob_analyzer(text):
                    ...
                    positive_words = [
                        'buy', 'bull', 'soar', 'growth', 'opportunity', 'magnificent', 'brilliant', 'steady',
                        'top', 'best', 'strong', 'outperform', 'upgrade', 'recommend', 'favorite', 'leading',
                        'dominant', 'innovative', 'breakthrough', 'revolutionary', 'transformative', 'promising',
                        'undervalued', 'bargain', 'attractive', 'compelling', 'conviction', 'long-term', 'sustainable'
                    ]
                    
                    negative_words = [
                        'sell', 'bear', 'dip', 'turmoil', 'risk', 'beaten-down', 'down', 'collapse', 'crash',
                        'warning', 'concern', 'caution', 'trouble', 'struggle', 'challenge', 'headwind', 'pressure',
                        'decline', 'drop', 'fall', 'plunge', 'slump', 'weakness', 'vulnerable', 'exposed', 'threat',
                        'overvalued', 'expensive', 'premium', 'bubble', 'speculative', 'uncertain', 'volatile'
                    ]
                    ...
                    confidence = (base_confidence + word_confidence + phrase_confidence + (1 - subjectivity)) / 4
            ...
            for item in news:
                try:
                    ...
                    if 'displayTime' in item:
                        news_date = datetime.fromisoformat(item['displayTime'].replace('Z', '+00:00'))
                    elif 'content' in item and 'displayTime' in item['content']:
                        news_date = datetime.fromisoformat(item['content']['displayTime'].replace('Z', '+00:00'))
    ...
    def _calculate_metrics(
        ...
        valid_confidences = [r["confidence"] for r in sentiment_results if r.get("confidence") is not None]
    ...
    async def get_news_data(
        ...
        try:
            ...
            for article in articles:
                try:
                    self.logger.debug(f"Processing article: {article.get('title', 'No title')}")
                    ...
                    if self.sentiment_analyzer is not None:
                        try:
                            ...
                            self.logger.debug(f"Sentiment analysis result: {sentiment} (confidence: {confidence})")
                        except Exception as e:
