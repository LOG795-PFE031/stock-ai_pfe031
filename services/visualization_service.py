...
class VisualizationService(BaseService):
    ...
    async def get_prediction_chart(
        ...
        try:
            ...
            prediction_result = await prediction_service.get_historical_predictions(symbol, days=days)
            ...
            self.logger.error(f"Error generating prediction chart for {symbol}: {str(e)}")
            return {
                "status": "error",
                "error": str(e)
            }
