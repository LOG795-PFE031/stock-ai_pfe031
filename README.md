...
Le système est organisé autour d'une architecture de monolithique conteneurisés avec Docker. Chaque composant est dédié à une fonction précise dans le pipeline d'analyse et de prédiction des marchés boursiers.
...
3. **⚙️ Services d’automatisation (`prefect-server`, `prefect-postgres`)**  
...
4. **📡 Service d'observabilité avec Prometheus & Grafana (`prometheus`, `grafana`)**
...
5. **🔐 Authentification & stockage associé (`auth-service`, `postgres`)**
   Service d’authentification en C# et base Postgres associée pour les utilisateurs/portfolio.

6. **📨 Messagerie inter‑services (`rabbitmq`)**
...
Tous les composants sont conteneurisés avec Docker.
